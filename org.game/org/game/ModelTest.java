...
public class ModelTest {
	...
	public void modelConstructor() throws InterruptedException {
		...
		assertEquals(model.getHouseList()[0].getPrice(), 500);
	}
	
	@Test
	public void buyHouseNoMoney() throws InterruptedException {
		Model model = new Model();
		model.getHouseList()[0].setPrice(99999);
		model.buyHouse(model.getHouseList()[0]);
		assertEquals(model.getHouseList()[0].getOwnershipState(), false);
	}
	
	@Test
	public void buyHouseOwned() throws InterruptedException {
		Model model = new Model();
		model.getHouseList()[0].setPrice(500);
		model.buyHouse(model.getHouseList()[0]);
		assertEquals(model.getHouseList()[0].getOwnershipState(), true);
		model.buyHouse(model.getHouseList()[0]);
		assertEquals(model.getHouseList()[0].getOwnershipState(), true);
	}
